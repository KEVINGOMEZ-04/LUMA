...
class LumaApp {
  ...
  async openSeriesDetail(seriesId) {
    let series = this.storage.getSeriesById(seriesId);
    if (!series) {
      const allSeries = this.storage.getSeries();
      series = allSeries.find(s => String(s.id) === String(seriesId) || String(s.tmdbId) === String(seriesId)) || allSeries[0];
    }
    if (!series) return;
    ...
    if (biblioSec) {
      ...
      biblioSec.classList.remove('active');
    }
    if (detailSec) {
    ...
    let totalPctSum = 0;
    const totalSeriesEpisodes = series.totalEpisodes || (series.seasons ? series.seasons.reduce((acc, s) => acc + (s.episodeCount || 0), 0) : 10);
    ...
    if ((!series.seasons || series.seasons.length <= 1) && series.tmdbId) {
      window.MediaService.getSeriesDetails(series.tmdbId).then(details => {
        if (details && details.seasons && details.seasons.length > 0) {
          ...
          this.storage.saveSeries(series);
          if (this.activeSeriesId === series.id) {
            this.renderSeriesSeasonTabs(series);
          }
        }
      }).catch(() => {});
  ...
  async renderSeriesEpisodes(series, seasonNumber) {
    ...
    if (!episodes && window.SERIES_PRELOADED_EPISODES) {
      const matchKey = series.id || series.tmdbId;
      episodes = window.SERIES_PRELOADED_EPISODES[matchKey]?.[seasonNumber] || window.SERIES_PRELOADED_EPISODES[String(series.tmdbId)]?.[seasonNumber];
      if (episodes) {
  ...
  _renderEpisodesDOM(series, seasonNumber, episodes) {
    const episodesListEl = document.getElementById('series-episodes-list');
    ...
    episodesListEl.innerHTML = episodes.map(ep => {
      ...
      // Comprobar si este capítulo es el punto seleccionado como "Visto hasta aquí" o anterior
      ...
      const isPastUpTo = (lastWatched.episode > 0) && (
        lastWatched.season > seasonNumber ||
        (lastWatched.season === seasonNumber && lastWatched.episode > ep.episodeNumber)
      );

      let upToBtnHtml = '';
      if (isUpToActive) {
        upToBtnHtml = `
          <button type="button" class="btn-ep-watch-upto is-active" onclick="window.app.watchEpisodesUpTo('${series.id}', ${seasonNumber}, ${ep.episodeNumber})">
            <span>Visto hasta aquí ✓</span>
          </button>
        `;
      } else if (isPastUpTo) {
        upToBtnHtml = `
          <button type="button" class="btn-ep-watch-upto is-past-disabled" disabled title="Capítulo ya incluido en tu avance">
            <span>Visto hasta aquí ✓</span>
          </button>
        `;
      } else {
      ...
      return `
        <div class="series-episode-card ${isWatched ? 'is-watched' : ''} ${isTarget ? 'is-current-target' : ''}" id="series-ep-card-${seasonNumber}-${ep.episodeNumber}" data-ep="${ep.episodeNumber}">
          ...
          <!-- Fila de Encabezado del Episodio: Número + Miniatura + Título -->
          <div class="series-ep-header-row">
            <!-- Número de Capítulo -->
            ...
            <!-- Título y Datos del Capítulo -->
            <div class="series-ep-title-box">
              <h4 class="series-ep-title" title="${window.Utils.sanitizeHTML(ep.name || `Capítulo ${ep.episodeNumber}`)}">${window.Utils.sanitizeHTML(ep.name || `Capítulo ${ep.episodeNumber}`)}</h4>
              <span class="series-ep-tag-meta">Capítulo ${ep.episodeNumber} · ${ep.duration || '45 min'}</span>
            </div>
          </div>

          <!-- Resumen / Sinopsis del Capítulo Optimizada a Ancho Completo -->
          <div class="series-ep-overview-wrap">
