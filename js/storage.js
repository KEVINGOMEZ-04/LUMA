...
(function() {
  ...
  class LumaStorage {
    ...
    updateChatGoalProgress(messageId, percentage) {
      const data = this.getGroupData();
      if (!data.messages) return null;
      const msg = data.messages.find(m => m.id === messageId);
      if (msg && msg.payload) {
        msg.payload.percentage = Math.max(0, Math.min(100, parseInt(percentage, 10) || 0));
        this.saveGroupData(null, data);
        this.notify('chatMessages');
      }
      return msg;
    ...
    restoreMockupChat() {
      ...
      if (group) {
        group.name = 'Grupo LUMA';
        group.icon = '☀️';
        group.motto = 'Buenas conversaciones, mejores recuerdos ✨';
        group.pinnedText = 'Próximo viaje: Guatapé — 18 Septiembre 🏞️';
        this.updateGroup(group.id, group);
      }
      this.notify('chatMessages');
      this.notify('groups');
      return true;
